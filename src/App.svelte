...
>
  {#if !$isReady || $isPreloadingFiles}
    <Preloader
      autoStart={!import.meta.env.PROD}
      cacheName="wo-transcribe"
      cacheVersion={import.meta.env.VITE_TRANSCRIBE_VERSION}
      urls={[
        import.meta.env.VITE_TRANSCRIBE_WASM,
        import.meta.env.VITE_TRANSCRIBE_WORKER,
        import.meta.env.VITE_TRANSCRIBE_MODEL,
      ]}
      ><Header title={config.titleHeader} class="!scale-75 !mb-0" />
      <p class="text-center mt-2 mb-6" style="max-width:50ch" slot="message">
        The app needs to download <strong>32M of data</strong> on first startup.<br
        />
        <span class="text-xs"
          >The data will be stored in the browser so you won't have to download
          it again (unless you clear the browser cache).</span
        >
      </p>
    </Preloader>
  {:else}
