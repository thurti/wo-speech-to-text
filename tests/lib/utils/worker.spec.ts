...
describe("lib/utils/worker", () => {
  ...
  describe("#isCached()", () => {
    ...
    it("should return true if the url is cached", async () => {
      vi.stubGlobal("caches", {
        match: vi.fn(() => Promise.resolve(true)),
      });

      const test = await isCached("test");
      expect(test).toBe(true);
      expect(caches.match).toHaveBeenCalledWith("test");
    });

    it("should return false if the url is not cached", async () => {
      vi.stubGlobal("caches", {
